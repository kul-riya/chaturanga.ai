...
export default function ChessBoardComponentPvC({ playerColour = "white" }) {
  ...
  const recordMoveToDatabase = async (move, newMoveNumber, eval_score, depth) => {
    if (!currentGameId) {
  ...
  const recordEngineMoveToDatabase = async (move, newMoveNumber) => {
    if (!currentGameId) {
      console.warn("No game ID, skipping database record");
      return;
    }
  ...
  const handleMove = async (from, to, promotionPiece = "q") => {
    ...
    // Record to database
    setIsComputer(false);
    await recordMoveToDatabase(move, newMoveNumber, 0, 0);
  ...
  const getPositionEvaluation = async () => {
    ...
    try {
      ...
      if (result && result.score !== undefined) {
        setPositionEval(-1 * result.score);
  ...
  useEffect(() => {
    ...
    const makeComputerMove = async () => {
      ...
      if (move) {
        ...
        await recordMoveToDatabase(move, newMoveNumber, positionEval, 4);
